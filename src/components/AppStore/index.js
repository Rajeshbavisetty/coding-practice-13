...
class AppStore extends Component {
  state = {list: appsList, searchInput: '', isActive: ''}
  changes = event => {
  ...
  getDetails = tabId => {
    const filterdList = appsList.filter(each => {
      return each.category === tabId
    })

    this.setState(prevstate => {
      return {list: filterdList}
    })
  ...
  render() {
    const {searchInput, list, isActive} = this.state
    const searchResults = list.filter(each => {
      return each.appName.toLowerCase().includes(searchInput.toLowerCase())
    })
    return (
      <div className="bg-container">
        <h1 className="header">App Store</h1>
        <div className="inputc">
          <input type="search" className="inputstyle" onChange={this.changes} />
          <img
            src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png"
            className="imgsize" alt="search icon"
          />
        </div>
          ...
          {tabsList.map(eachDetails => {
            return (
              <TabItem
                ...
                key={eachDetails.tabId}
                getDetails={this.getDetails}
               click={isActive === eachDetails.tabId}
              />
            )
          })}
        </ul>
        <ul className="container">
          {searchResults.map(each => {
            return <AppItem each={each} key={each.appId} />
          })}
        </ul>
      </div>
    )
